...
Vision-language models can score robot videos cheaply, but episode accuracy does not determine which policy is superior, practically equivalent, or still unresolved, or how much human evidence is enough. Structured evaluator errors can look small on average while reversing a matched policy effect.
...
This is a proposal-level novelty claim, not universal priority. A systematic review remains a prerequisite for publication.
